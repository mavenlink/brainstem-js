$ = require 'jquery'
underscore = require './utility-functions'
...
module.exports = class Collection extends Backbone.Collection
  ...
  @pickFetchOptions: (options) ->
    underscore.pick options, @OPTION_KEYS
  ...
  fetch: (options) ->
    options = if options then underscore.clone(options) else {}
    ...
    loader = @storageManager.loadObject(options.name, underscore.extend({}, @firstFetchOptions, options))
  ...
  refresh: (options = {}) ->
    @fetch underscore.extend(@lastFetchOptions, options, cache: false)
  ...
  update: (models, options = {}) ->
    addOpts = underscore.pick(options, 'silent')
  ...
  reload: (options) ->
    ...
    @reset [], silent: true
    @setLoaded false
    loadOptions = underscore.extend({}, @lastFetchOptions, options, page: 1, collection: this)
    @storageManager.loadCollection @lastFetchOptions.name, loadOptions

  loadNextPage: (options = {}) ->
    if underscore.isFunction(options.success)
      success = options.success
      delete options.success

    @getNextPage(underscore.extend(options, add: true)).done(=> success?(this, @hasNextPage()))
  ...
  getPage: (index, options = {}) ->
    ...
    options = underscore.extend(options, @lastFetchOptions)
    ...
    @fetch underscore.extend(options, { reset: true })
  ...
  toServerJSON: (method) ->
    @map (model) -> underscore.extend(model.toServerJSON(method), id: model.id)
  ...
  _maxOffset: ->
    ...
    Utils.throwError('(pagination) you must define limit when using offset') if underscore.isUndefined(limit)
    limit * Math.ceil(@getServerCount() / limit) - limit

  _maxPage: ->
    perPage = @lastFetchOptions.perPage
    Utils.throwError('(pagination) you must define perPage when using page') if underscore.isUndefined(perPage)
